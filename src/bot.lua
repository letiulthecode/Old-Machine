...
local commands = { -- Define commands its a table that will contain our commands
    [prefix..'pingpong'] = { -- Indexes can also be strings! So the string index it will concat our prefix and the command name!
      ...
      if message.content:sub(1, #msg) == msg then
        if message.member:hasPermission('manageMessages') then
          ...
          if not cha:bulkDelete(message.channel:getMessagesBefore(message.id, repla)) then
            message:reply("Something went wrong whiling starting a purge...")
          else
            message:delete()
            message:reply("Chat purged by <@!"..message.author.id.."> (Cleared "..repla.." Messages)!")
          end
        else
...
client:on("ready", function() -- Remember we made a client variable? We can use it now!
    ...
    -- TODO: Remember very thing related with client, its related to the bot!
    print(os.date("%F %T", os.time()).." | \027[94m[BOT]\027[0m     | "..client.user.username.." Is online!")
    client:setGame("aye i re-encarned from Cyber.11!") -- Status message
end)
